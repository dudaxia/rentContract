(function($, undefined) {
    ...
    var uploadImage = function(selector, options) {
        ...
        self.maxsize = 100 * 1024;
        self.compressWidth=900;
    ...
    uploadImage.prototype = {
        init: function(selector, type) {
            ...
            if (!!type) {
                ...
                if (self.options) {
                    if (!!self.options.fileImage && self.options.fileImage.length > 0) {
                        ...
                        var list = [];
                        console.log( "self.options.fileImage",self.options.fileImage )
            ...
            self.$el.on('change', "input[type='file']", function() {
                ...
                oFReader.readAsDataURL(selfFile[0]);
                oFReader.onload=function () {
                    ...
                    img.onload = function () {
                        ...
                        //     self.corsUpload(that.files[0], (function($el) {
                        //         var dom = $el
                        ...
                        //     return;
                        // }
                        // console.log( "self.compressWidth",self.compressWidth )
                        var base64=self.compressImage(img,self.compressWidth);
                        console.log("compress_base64",base64.length);
                        parent = $(that).parents("div.upload");
                        self.corsUpload(that.files[0], (function($el) {
                            var dom = $el;
                            return function(file, data, resp) {
                                if (data && data.res.msg) {
                                    self.uploadSuccessDone(dom, file, data, resp, base64);
                                }
                            }
                        })(parent),base64);
                    };
        ...
        corsUpload: function(file, callback,base64) {
            console.log( "file0000",file );
        ...
        compressImage:function (img,compressWidth) {
            ...
            // self.ctx.clearRect(0, 0, imgWidth, imgHeight);
            // self.ctx.fillStyle = "#fff";
            // self.ctx.fillRect(0, 0, imgWidth, imgHeight);
            // self.ctx.drawImage(img,0,0,imgWidth,imgHeight);
            // var base64 = self.canvas.toDataURL("image/jpeg",0.98);
            // return base64;
        },
        uploadSuccessDone: function($el, file, data, resp , base64) {
            // var filePath = data.res.data.fileAccessPath,
            // var filePath = base64 || data.res.data.fileAccessPath,
            // var filePath = data.message,self = this;
            console.log( "filePath",filePath )
            var filePath = data.res.msg , self = this;

        ...
        success: function (jsonData) {
            ...
            if (jsonData && jsonData.code && jsonData.res && jsonData.res.data) {
                ...
                if ("0" == jsonData.code || "420" == jsonData.code || "421" == jsonData.code) {
                    ...
                    var leaseDate = data.house.leaseDate;
                    ...
                    if (lastNeedPay&&(jsonData.code!="421")) {
                        var $li = '<li class="last-need-pay clearfix">'+
                                    '<div class="isPayOk">待支付</div>'+
                                    '<div class="pay-content-info">'+
                                        '<div class="lastNeedPayDate">'+ lastNeedPay.planRepaymentDate.split(" ")[0] +'</div>'+
                                        '<div class="pay-info rentMoneyDate clearfix">'+
                                            '<p class="floatleft rentPayMoney">总租金:￥<span class="pay-deal-amount">'+ lastNeedPay.planRepaymentAmount +'</span></p>'+
                                            '<p class="floatright rentPayAllDate"><span class="all-rent-date">'+ lastNeedPay.planRepaymentDate.split(" ")[0]+"-"+ lastNeedPay.PlanRepaymentEndDate.split(" ")[0] +'</span></p>'+
                                        '</div>'+
                                        '<div class="linef2"></div>'+
                                        '<div class="pay-info rentTypeRealDate clearfix">'+
                                            '<p class="floatleft rentPayType">请在交租日前一个月内进行支付</p>'+
                                        '</div>'+
                                    '</div>'+
                                '</li>';
                    ...
                    if($.isArray(payHistory)
                       ...
                       $.each(payHistory, function(i, j){
                            ...
                            var $li = '<li class="clearfix">'+
                                        '<div class="isPayOk">已支付</div>'+
                                        '<div class="pay-content-info">'+
                                            '<div class="lastNeedPayDate">'+ j.planRepaymentDate.split(" ")[0] +'</div>'+
                                            '<div class="pay-info rentMoneyDate clearfix">'+
                                                '<p class="floatleft rentPayMoney">总租金:￥<span class="pay-deal-amount">'+ getRepayAmount( j.planRepaymentAmount,j.realRepaymentAmount,j.planStatus ) +'</span></p>'+
                                                '<p class="floatright rentPayAllDate"><span class="all-rent-date">'+ j.planRepaymentDate.split(" ")[0]+"-"+ j.planRepaymentEndDate.split(" ")[0] +'</span></p>'+
                                            '</div>'+
                                            '<div class="linef2"></div>'+
                                            '<div class="pay-info rentTypeRealDate clearfix">'+
                                                '<p class="floatleft rentPayType">方式：<span class="pay-stauts">'+ getPayStatus(j.planStatus) +'</span></p>'+
                                                '<p class="floatright rentPayRealDate"><span class="real-pay-date">'+ j.repaymentDate +'</span></p>'+
                                            '</div>'+
                                        '</div>'+
                                    '</li>';
                            $("ul[name=payHistory]").append($li);
